...
class HashtagViewSetTestCase(TestCase):
    ...
    def test_retrieve_hashtag(self):
        response = self.client.get(
            reverse("post:hashtag-detail", kwargs={"pk": self.hashtag1.id})
        )
    ...
    def test_update_hashtag(self):
        data = {"name": "updated_hashtag"}
        response = self.client.put(
            reverse("post:hashtag-detail", kwargs={"pk": self.hashtag1.id}), data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            Hashtag.objects.get(id=self.hashtag1.id).name, "updated_hashtag"
        )
...
class PostViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.post1 = Post.objects.create(
            author=self.user, title="Test Post 1", content="Test content 1"
        )
        self.post1.hashtag.add(self.hashtag1)
        self.post2 = Post.objects.create(
            author=self.user, title="Test Post 2", content="Test content 2"
        )
    ...
    def test_create_post(self):
        ...
        self.assertEqual(
            list(Post.objects.last().hashtag.all()), [self.hashtag1, self.hashtag2]
        )

    def test_retrieve_post(self):
        response = self.client.get(
            reverse("post:post-detail", kwargs={"pk": self.post1.id})
        )
    ...
    def test_update_post(self):
        ...
        response = self.client.put(
            reverse("post:post-detail", kwargs={"pk": self.post1.id}), data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Post.objects.get(id=self.post1.id).title, "Updated Post")
        self.assertEqual(
            list(Post.objects.get(id=self.post1.id).hashtag.all()), [self.hashtag1]
        )
